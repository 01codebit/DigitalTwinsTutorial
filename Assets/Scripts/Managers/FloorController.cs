...
{
    [SerializeField] private GameObject _exteriorBuilding;
    ...
    private void OnSceneLoaded(Scene scene, LoadSceneMode mode)
    {
        ...
        {
            _exteriorCamera = GameObject.FindWithTag("ExteriorCam")?.GetComponent<CinemachineCamera>();
        }

        if (_topDownCamera == null)
        {
            _topDownCamera = GameObject.FindWithTag("TopDownCam")?.GetComponent<CinemachineCamera>();
        }
        ...
        if (_exteriorBuilding == null)
        {
            foreach (var rootObject in scene.GetRootGameObjects())
            {
                if (rootObject.CompareTag("ExteriorBuilding"))
                {
                    ...
                    Debug.Log($"[FloorController] Found exterior building in scene '{scene.name}'.");
                    break;
                }
            }
        }
